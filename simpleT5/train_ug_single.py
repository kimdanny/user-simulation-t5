...
if os.path.exists(os.path.join(Path(os.path.dirname(os.path.realpath(__file__))), 'utt_no-alt', DATASET)):
    clear_confirmation = input(f"Do you want to clear '{os.path.join(Path(os.path.dirname(os.path.realpath(__file__))), 'utt_no-alt', DATASET)}' ?? [yes/no]")
    if clear_confirmation == 'yes':
        shutil.rmtree(os.path.join(Path(os.path.dirname(os.path.realpath(__file__))), 'utt_no-alt', DATASET))
    else:
        pass

output_dir_path = os.path.join(Path(os.path.dirname(os.path.realpath(__file__))), 'utt_no-alt', DATASET, 'output')
best_model_dir_path = os.path.join(output_dir_path, 'best_model')

predictions_dir_path = os.path.join(Path(os.path.dirname(os.path.realpath(__file__))), 'utt_no-alt', DATASET, 'predictions')
results_dir_path = os.path.join(Path(os.path.dirname(os.path.realpath(__file__))), 'utt_no-alt', DATASET, 'results')

df = pd.read_csv(dataset_path, index_col=False).astype(str)
df = transform_df(df)
df = df.sample(frac=1).reset_index(drop=True)
df = df[df['prefix'] == 'utterance generation']
